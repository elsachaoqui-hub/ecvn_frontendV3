...
/** 5.1 預結算：依 15 分鐘時段 × 節點彙總（不含各電號拆分） */
export type TimeSlotAggregatedFlowRow = {
  dateLabel: string;
  timeLabel: string;
  sourceNode: string;
  targetNode: string;
...
export function aggregateSankeyFlowsByTimeSlot(flows: SankeyFlowRow[]): TimeSlotAggregatedFlowRow[] {
  const map = new Map<string, TimeSlotAggregatedFlowRow>();
  for (const f of flows) {
    const key = `${f.dateLabel}|${f.timeLabel}|${f.sourceNode}|${f.targetNode}`;
  ...
  return Array.from(map.values())
    .map((row) => ({ ...row, flowKwh: Number(row.flowKwh.toFixed(3)) }))
    .sort((a, b) => {
      const t = `${a.dateLabel} ${a.timeLabel}`.localeCompare(`${b.dateLabel} ${b.timeLabel}`);
      if (t !== 0) return t;
      const n = a.sourceNode.localeCompare(b.sourceNode, 'zh-Hant');
      if (n !== 0) return n;
      return a.targetNode.localeCompare(b.targetNode, 'zh-Hant');
    });
}
