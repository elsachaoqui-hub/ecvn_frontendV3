...
import { ALL_AGENTS_ID, agentFilterLabel } from '@/lib/marketMonitoringData';
import {
...
export default function SettlementPreSettlementPage({
  ...
  const [selectedAgentId, setSelectedAgentId] = useState<string>(() => String(agents[0]?.id ?? ALL_AGENTS_ID));
  ...
  return (
    <div className="space-y-6 pb-8 text-slate-800">
      <section className="rounded-2xl border border-indigo-200 bg-gradient-to-br from-indigo-50/80 to-white p-4 shadow-sm sm:p-5">
          ...
          <div className="mt-3 rounded-xl border border-slate-200/90 bg-white/95 p-3 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-12 lg:items-end">
              {variant === 'pre' ? (
                <div className="sm:col-span-2 lg:col-span-4">
                  <label className="mb-1 block text-ui-10 font-bold text-slate-600">代理人</label>
                  <select
              ...
              <div className={variant === 'pre' ? 'lg:col-span-2' : 'lg:col-span-3'}>
            ...
            <p className="mt-2 text-ui-10 font-semibold text-slate-500">
              ...
              {variant === 'pre' ? ` · ${agentFilterLabel(selectedAgentId)}` : ''}
            </p>
          </div>
      ...
      <section className="rounded-2xl border border-slate-300 bg-white p-5 shadow-sm">
        ...
        <div className="mb-5 mt-4 rounded-2xl border border-slate-300 bg-white p-4 shadow-sm">
          ...
          <p className="mb-2 text-xs font-semibold text-slate-600">
            ...
            分鐘可編輯量測值並填寫原因；異常以紅色標示，廠商確認後改為綠色。表格數值與上方桑基圖節點／連線可點擊，開啟
            {variant === 'pre'
              ? '以 15 分鐘時段為主之節點彙總流向明細（不含各電號拆分）。'
              : ' G1～G5、L1～L5 與流向組成明細。'}
          </p>
      ...
      <SankeyDetailDialog focus={detailFocus} onClose={() => setDetailFocus(null)} variant={variant} />
      {saveToast ? (
